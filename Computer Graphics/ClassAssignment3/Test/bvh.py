...
def drawJoint():
    global root_joint, joint_num, hierarchy, hierarchy_num, offset, motions, frame_num, frame_time, flag, pl
    ...
    for i in range(hierarchy_num):
        if hierarchy[i] == '{':
            ...
            if hierarchy[i+1] == '}':
                ...
                if int(motions[u][0]) == 1:
                    if int(motions[u+1][0]) == 2 and int(motions[u+2][0]) == 3:
                        glTranslatef(float(motions[u][pl]), float(motions[u+1][pl]), float(motions[u+2][pl]))
                            
                    elif int(motions[u+1][0]) == 3 and int(motions[u+2][0]) == 2:
                        glTranslatef(float(motions[u][pl]), float(motions[u+2][pl]), float(motions[u+1][pl]))

                if int(motions[u][0]) == 2:
                    if int(motions[u+1][0]) == 1 and int(motions[u+2][0]) == 3:
                        glTranslatef(float(motions[u+1][pl]), float(motions[u][pl]), float(motions[u+2][pl]))
            
                    elif int(motions[u+1][0]) == 3 and int(motions[u+2][0]) == 1:
                        glTranslatef(float(motions[u+2][pl]), float(motions[u][pl]), float(motions[u+1][pl]))
                
                if int(motions[u][0]) == 3:
                    if int(motions[u+1][0]) == 1 and int(motions[u+2][0]) == 2:
                        glTranslatef(float(motions[u+1][pl]), float(motions[u+2][pl]), float(motions[u][pl]))
                    
                    elif int(motions[u+1][0]) == 2 and int(motions[u+2][0]) == 1:
                        glTranslatef(float(motions[u+2][pl]), float(motions[u+1][pl]), float(motions[u][pl]))


                if int(motions[u+3][0]) == 4:
                    if int(motions[u+4][0]) == 5 and int(motions[u+5][0]) == 6:
                        glRotatef(float(motions[u+3][pl]), 0,0,1)
                        glRotatef(float(motions[u+4][pl]), 1,0,0)
                        glRotatef(float(motions[u+5][pl]), 0,1,0)
                                        
                    elif int(motions[u+4][0]) == 6 and int(motions[u+5][0]) == 5:
                        glRotatef(float(motions[u+3][pl]), 0,0,1)
                        glRotatef(float(motions[u+4][pl]), 0,1,0)
                        glRotatef(float(motions[u+5][pl]), 1,0,0)

                if int(motions[u+3][0]) == 5:
                    if int(motions[u+4][0]) == 4 and int(motions[u+5][0]) == 6:
                        glRotatef(float(motions[u+3][pl]), 1,0,0)
                        glRotatef(float(motions[u+4][pl]), 0,0,1)
                        glRotatef(float(motions[u+5][pl]), 0,1,0)
            
                    elif int(motions[u+4][0]) == 6 and int(motions[u+5][0]) == 4:
                        glRotatef(float(motions[u+3][pl]), 1,0,0)
                        glRotatef(float(motions[u+4][pl]), 0,1,0)
                        glRotatef(float(motions[u+5][pl]), 0,0,1)
                
                if int(motions[u+3][0]) == 6:
                    if int(motions[u+4][0]) == 4 and int(motions[u+5][0]) == 5:
                        glRotatef(float(motions[u+3][pl]), 0,1,0)
                        glRotatef(float(motions[u+4][pl]), 0,0,1)
                        glRotatef(float(motions[u+5][pl]), 1,0,0)
                    
                    elif int(motions[u+4][0]) == 5 and int(motions[u+5][0]) == 4:
                        glRotatef(float(motions[u+3][pl]), 0,1,0)
                        glRotatef(float(motions[u+4][pl]), 1,0,0)
                        glRotatef(float(motions[u+5][pl]), 0,0,1)
                ...
                if int(motions[u][0]) == 4:
                    if int(motions[u+1][0]) == 5 and int(motions[u+2][0]) == 6:
                        glRotatef(float(motions[u][pl]), 0,0,1)
                        glRotatef(float(motions[u+1][pl]), 1,0,0)
                        glRotatef(float(motions[u+2][pl]), 0,1,0)
                    
                    elif int(motions[u+1][0]) == 6 and int(motions[u+2][0]) == 5:
                        glRotatef(float(motions[u][pl]), 0,0,1)
                        glRotatef(float(motions[u+1][pl]), 0,1,0)
                        glRotatef(float(motions[u+2][pl]), 1,0,0)
                            
                            
                if int(motions[u][0]) == 5:
                    if int(motions[u+1][0]) == 4 and int(motions[u+2][0]) == 6:
                        glRotatef(float(motions[u][pl]), 1,0,0)
                        glRotatef(float(motions[u+1][pl]), 0,0,1)
                        glRotatef(float(motions[u+2][pl]), 0,1,0)

                    elif int(motions[u+1][0]) == 6 and int(motions[u+2][0]) == 4:
                        glRotatef(float(motions[u][pl]), 1,0,0)
                        glRotatef(float(motions[u+1][pl]), 0,1,0)
                        glRotatef(float(motions[u+2][pl]), 0,0,1)
        
                if int(motions[u][0]) == 6:
                    if int(motions[u+1][0]) == 4 and int(motions[u+2][0]) == 5:
                        glRotatef(float(motions[u][pl]), 0,1,0)
                        glRotatef(float(motions[u+1][pl]), 0,0,1)
                        glRotatef(float(motions[u+2][pl]), 1,0,0)
                    
                    elif int(motions[u+1][0]) == 5 and int(motions[u+2][0]) == 4:
                        glRotatef(float(motions[u][pl]), 0,1,0)
                        glRotatef(float(motions[u+1][pl]), 1,0,0)
                        glRotatef(float(motions[u+2][pl]), 0,0,1)
